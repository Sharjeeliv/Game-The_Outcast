...
class SoundEngine:
    ...
    def background_music(self):
        ...
        if Game_Data[0] == 'null' and self.initialize_music:
            pygame.mixer.music.load('assets/audio/music/music_1.mp3')
            pygame.mixer.music.play(-1)
            self.initialize_music = False

        elif Game_Data[0] == 'play' and self.initialize_music:
            pygame.mixer_music.stop()
            pygame.mixer.music.load('assets/audio/music/music_10.mp3')
            pygame.mixer.music.play(-1)
            self.initialize_music = False
            ...
            if Game_Data[7] < 300:
                pygame.mixer_music.stop()
                pygame.mixer.music.load('assets/audio/music/music_9.mp3')
                pygame.mixer.music.play(-1)
                self.initialize_music = False
            else:
                pygame.mixer_music.stop()
                pygame.mixer.music.load('assets/audio/music/music_6.mp3')
                pygame.mixer.music.play(-1)
                self.initialize_music = False

        elif Game_Data[0] == 'castle' and self.initialize_music:
            pygame.mixer_music.stop()
            pygame.mixer.music.load('assets/audio/music/music_7.mp3')
            pygame.mixer.music.play(-1)
            self.initialize_music = False
...
class UserInterface(SoundEngine):
    ...
    def __init__(self):
        ...
        # Other menu screens
        self.instructions = image.load('assets/interface/gui/Instructions.png')
        self.instructions = transform.scale(self.instructions, (1280, 720))
        self.credits = image.load('assets/interface/gui/credits.png')
        self.credits = transform.scale(self.credits, (1280, 720))

        self.game_over_image = image.load('assets/interface/gui/Game_Over.png')
        self.game_over_image = transform.scale(self.game_over_image, (1280, 720))
        self.you_win_image = image.load('assets/interface/gui/You_Win.png')
        self.you_win_image = transform.scale(self.you_win_image, (1280, 720))
        self.story_image = image.load('assets/interface/gui/Story.png')
        ...
        '''
        # Game window
        icon = image.load('assets/interface/gui/game_icon.png')
        ...
        for i in range(35):  # Frames in a sequence
            ui_graphics = image.load('assets/interface/graphics/image_' + str(i) + '.png')
    ...
    def title_screen(self, screen, mouse_x, mouse_y, mouse_b, sound_class):
        ...
        menu_background = image.load('assets/interface/gui/Title_Screen/Menu.png')
        menu_background = transform.smoothscale(menu_background, (1280, 720))
        title = image.load('assets/interface/gui/Title_Screen/Title.png')
        ...
        # -------------------------------------------------
        for i in range(1, 4):
            subtitle = image.load('assets/interface/gui/Title_Screen/image_' + str(i) + '.png')
            subtitle_pressed = image.load('assets/interface/gui/Title_Screen/image_' + str(i) + '_P.png')
...
class WorldMechanics(SoundEngine):
    ...
    def __init__(self, speed):
        ...
        self.overlay = image.load('assets/World/Overlay.png')
        ...
        self.island = image.load('assets/World/Island/Island.png')
        self.island = transform.scale(self.island, (5120, 2880))

        self.mask = image.load('assets/World/Island/Island_Mask.png')
        ...
        # -------------------------------------------------
        for i in range(1, 3):
            castle_object = image.load('assets/World/Castle/Objects/Object_' + str(i) + '.png')
    ...
    def __init__(self):
        ...
        for level in range(1, 6):
            ...
            for level_status in range(1, 3):
                # -------------------------------------------------
                level_image = image.load('assets/World/Castle/Levels/Level_' + str(level) + '_' + str(level_status) + '.png')
                level_mask = image.load('assets/World/Castle/Masks/Mask_' + str(level) + '_' + str(level_status) + '.png')
...
class EnemyMechanics(SoundEngine):
    def __init__(self):
        ...
        # -------------------------------------------------
        for i in range(0, 2):
            enemy = image.load('assets/World/Enemy/Enemy_' + str(i) + '.png')
            enemy_mask = image.load('assets/World/Enemy/Mask_' + str(i) + '.png')
        ...
        # Rock
        self.rock_mask = image.load('assets/World/Castle/Objects/Mask_2.png')
        self.rock = image.load('assets/World/Castle/Objects/Object_2.png')
        self.rock_data = []
        self.rock_mask_data = []
    ...
    def init_boss_battle(self, sound_class):
        if not self.boss_fight:
            ...
            # -------------------------------------------------
            for i in range(1, 11):
                boss_frame = image.load('assets/World/Enemy/Boss/Boss_' + str(i) + '.png')
...
class PlayerMechanics(Castle):
    ...
    def __init__(self):
        ...
        self.shadow = image.load('assets/player/player_walking/Player_Shadow.png')
        ...
        for walk_cycle in range(1, 9):  # All motion types
            ...
            for frames in range(1, 9):  # Frames in a sequence
                frame_image = image.load(
                    'assets/player/player_walking/Player_Walking_' + str(walk_cycle) + '/Player_F' + str(frames) + ".png")
                movement_package.append(frame_image)
            self.image.append(movement_package)

        player_shooting_right = image.load('assets/player/player_shooting/player_shooting.png')
